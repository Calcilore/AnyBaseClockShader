...
bool doNumbers(vec2 uv) {
    // number to render
    float base = 10.0;//float(int(iDate.z) % 10);
    float scale = 1.0;
    float digitWidth = 0.245 * scale;

    int digitCountMS = int(logx(base, 60.0))+1;
    int digitCountH = int(logx(base, 24.0))+1;

    // seconds, minutes, hours
    float[] numbers = float[](mod(iDate.w, 60.0), mod(iDate.w / 60.0, 60.0), iDate.w / 60.0 / 60.0);

    float number = mod(iDate.w, 60.0) + (mod(floor(iDate.w / 60.0), 60.0)*pow(base, floor(logx(base, 60.0))+1.0)) + (mod(floor(iDate.w / 60.0 / 60.0), 24.0)*pow(base, floor(logx(base, 60.0*60.0))+1.0));
    int digits = int(logx(base, float(number)))+1;
    float digitPos = uv.x;

    // which number to render
    {
        float halfMinuteWidth = float(digitCountMS) * digitWidth * 0.5;
        float hourWidth = float(digitCountH) * digitWidth;
        if (uv.x < -halfMinuteWidth) { // Hours
            col = vec3(1.0, 0.0, 0.0);
            // OOB
            if (uv.x < -halfMinuteWidth - hourWidth) {
                return false;
            }

            number = numbers[2];
            digits = digitCountH;
            digitPos = uv.x + halfMinuteWidth + hourWidth;
        } else if (uv.x > halfMinuteWidth) { // Seconds
            col = vec3(0.0, 0.0, 1.0);
            // OOB
            if (uv.x > halfMinuteWidth * 3.0) {
                return false;
            }
    ...
    int digitIndex = digits-1 - int(digitPos/digitWidth);
    ...
    // draw the digit
    vec2 drawPos = vec2(digitWidth*(float(digits-digitIndex)-0.5), 0.0);
